...
| Caption `\vskip` 10 pt → 3 pt (override `\@makecaption`), `\belowcaptionskip` → 0 pt | ~1 line per float | matches corpus caption spacing; `\abovecaptionskip` is ignored by spconf, so the override is the only way in |
...
Measured on one 5-page paper with 14 headings. The middle row is not a value
any paper publishes — no accepted paper ships its source — it is a setting
derived by back-solving ≈4.3 pt per ex from the other two builds so the printed
white would land on the corpus floor, then rebuilt and re-measured (it came out
at 11.04 / 5.34, just above the 10.5 / 4.3 floor).

| | pages | body text lines | extra white above / below a heading |
|---|---|---|---|
| stock (`-3.5ex/2.3ex`, `-3.25ex/1.5ex`) | **6** | 1002 | 15.12 / 8.20 pt |
| back-solved to ≈ the corpus floor (`-2.5ex/1.3ex`, `-2.3ex/0.9ex`) | **6** | 1002 | 11.04 / 5.34 pt |
...
4. Expanding abbreviated venue names back to full form *is* a legitimate fill
   lever, and a large one — it was worth 18 lines in the shrink direction on a
...
Emphasis is markup around the author's words, so it is a **proposal**, never an
edit you make. Measured on the corpus (pages 1–4, title block and Index Terms
excluded), italic runs in the body number 0 / 1 / 8 / 11 / 14 / 27 / 29 — there
is no venue norm, so "the venue does/doesn't italicise" is never an argument.
The one pattern worth reporting is the *kind*: the corpus quotes an example
string taken from data (`"black pants"`), and reserves italics for defined
terms and method names. A paper that italicises quoted example strings reads
differently from its neighbours; say that, show the instances, and stop.
